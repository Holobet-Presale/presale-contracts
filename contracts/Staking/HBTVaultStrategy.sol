...
// File: contracts/Staking/HBTVaultStrategy.sol
...
contract StratManager is Ownable, Pausable {
    ...
    function setVault(address _vault) external onlyOwner {
        require(vault == address(0), "can't modify vault");
        vault = _vault;
    }
}
...
contract HBTVaultStrategy is StratManager {
    ...
    function harvest() public whenNotPaused {
        ...
        uint256 wHBTBal = IERC20(wHBT).balanceOf(address(this));
        IWHBT(address(wHBT)).withdraw(wHBTBal);
        if (wHBTBal > 0) {
            deposit();
        }
