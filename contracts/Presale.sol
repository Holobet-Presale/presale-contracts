...
contract HBTPresale is Ownable {
    ...
    event onClaim(uint256 hbtAmount, uint256 receiptTokenAmount);
    event onParticipate(
        uint256 hbtAmount,
        uint256 usdcAmount,
        address referrer
    ...
    function getPendingReward(address user) public view returns (uint256) {
        if (purchaseInfo[user].vaultReceiptAmount == 0) {
            return 0;
        }
        uint256 stakedFinalBalance = purchaseInfo[user].vaultReceiptAmount *
